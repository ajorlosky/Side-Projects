...
public class GameGUI extends JComponent {
	...
	public GameGUI(int timerDelayInMilliSecs, Game gameModelIn) {
		...
		setPreferredSize(
				new Dimension(gameModel.getMaxCols() * CELL_DIMENSION, gameModel.getMaxRows() * CELL_DIMENSION));
	...
	protected void paintComponent(Graphics g) {
		...
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[row].length; col++) {
				...
				g2.fill3DRect(col * CELL_DIMENSION, row * CELL_DIMENSION, CELL_DIMENSION, CELL_DIMENSION, RAISED_CELL);
			}
		}
	}
	...
	public static void main(String[] args) {
		Runnable createShowGUI = new Runnable() {
			public void run() {
				...
				int timerDelayInMilliSecs = Integer
						.parseInt(JOptionPane.showInputDialog(null, "Enter speed in milliseconds", 4000));
